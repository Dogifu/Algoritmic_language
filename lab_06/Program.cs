...
class MainProgram
{
        ...
        var Collection = new List<Weather>
    {
        ...
        new Weather() { Country = "RU", Name = "Aykhal", Temp = 261.85, Description = "few clouds" },
        ...
        new Weather() { Country = "TH", Name = "Thailand", Temp = 297.12, Description = "rain" },
        ...
        new Weather() { Country = "CN", Name = "Qiaotou", Temp = 299.27, Description = "rain" } 
       ...
       /* foreach (Weather weather in Collection)
        ...
        var x = from w in Collection.Take(c)
               where w.Description == "clear sky"
                 select  w;
        int l = (from w in Collection.Take(c)
                where w.Description == "clear sky"
                select w).Count();


        foreach (var w in x.SkipLast(l - 1))
       {
           Console.WriteLine("Первая найденная страна в которой погодные условия \"clear sky\" : " + w.Country + " " + w.Name);
       }

        Console.WriteLine();

        var q = from w in Collection.Take(c)
                where w.Description == "rain"
                select w;
        int p = (from w in Collection.Take(c)
                 where w.Description == "rain"
                 select w).Count();


        foreach (var w in q.SkipLast(p - 1))
        {
            Console.WriteLine("Первая найденная страна в которой погодные условия \"rain\" : " + w.Country + " " + w.Name);
        }

        Console.WriteLine();

        var a1 = from w in Collection.Take(c)
                where w.Description == "few clouds"
                select w;
        int a2 = (from w in Collection.Take(c)
                 where w.Description == "few clouds"
                 select w).Count();


        foreach (var w in a1.SkipLast(a2 - 1))
        {
            Console.WriteLine("Первая найденная страна в которой погодные условия \"few clouds\" : " + w.Country + " " + w.Name);
        } 
